...
app.post("/api/uploads", requireAdmin, upload.array("images", 12), async (req, res) => { const urls = []; for (const file of req.files || []) { const id = `${Date.now()}-${crypto.randomUUID()}`; const extension = path.extname(file.originalname).toLowerCase() || ".jpg"; await fs.writeFile(path.join(originalsDir, `${id}${extension}`), file.buffer); if (file.mimetype === "application/pdf") urls.push(`/uploads/originals/${id}${extension}`); else { await sharp(file.buffer).rotate().resize({ width: 1600, height: 1600, fit: "inside", withoutEnlargement: true }).webp({ quality: 82 }).toFile(path.join(optimizedDir, `${id}.webp`)); urls.push(`/uploads/optimized/${id}.webp`); } } res.status(201).json({ urls }); });

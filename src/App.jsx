...
function Corporate() { return <main className="corporate page-main"><PageHero eyebrow="KURUMSAL" title={<>Güven, bilgi ve <em>sorumlulukla</em> inşa ediyoruz.</>}>Kuzeykale'nin kurum kültürü, ekibi ve çalışma yaklaşımı.</PageHero><section className="corporate-menu-grid"><Link to="/kurumsal/hakkimizda"><b>01</b><h2>Hakkımızda</h2><span>Şirketimizi tanıyın →</span></Link><Link to="/kurumsal/is-ortaklari"><b>02</b><h2>İş Ortaklarımız</h2><span>Markaları görün →</span></Link><Link to="/kurumsal/ekibimiz"><b>03</b><h2>Ekibimiz</h2><span>Ekibimizi tanıyın →</span></Link><Link to="/kurumsal/e-katalog"><b>04</b><h2>E-Katalog</h2><span>Belgeleri inceleyin →</span></Link><Link to="/kurumsal/insan-kaynaklari"><b>05</b><h2>İnsan Kaynakları</h2><span>Ekibimize katılın →</span></Link></section></main>; }
function About() { const { company } = useContent(); return <main className="corporate page-main"><PageHero eyebrow="KURUMSAL · HAKKIMIZDA" title={<>Biz <em>kimiz?</em></>}/><section className="about-story"><div><h2>Kuzeykale İnşaat</h2><p>{company.about}</p><p>Taahhüt, mimarlık, mühendislik ve yapı denetim çalışmalarında kaliteyi, teknik disiplini ve güveni merkeze alıyoruz.</p></div><div className="about-gallery"><figure className="about-gallery-featured"><img src="/imported/about-office-team.webp" alt="Kuzeykale İnşaat yetkilileri ve ofis ekibi"/><figcaption>Yetkililerimiz ve ofis ekibimiz</figcaption></figure><figure><img src="/imported/about-ahmet-cevdet-desk.webp" alt="Ahmet Cevdet Uyanık çalışma masasında"/><figcaption>Ahmet Cevdet Uyanık · İnşaat Mühendisi</figcaption></figure><figure><img src="/imported/about-mehmet-cevad-desk.webp" alt="Mehmet Cevad Uyanık çalışma masasında"/><figcaption>Mehmet Cevad Uyanık · Makina Mühendisi</figcaption></figure></div></section></main>; }
...
function Team() { const { team = [] } = useContent(); return <main className="corporate page-main"><PageHero eyebrow="KURUMSAL · EKİBİMİZ" title={<>Deneyimli <em>ekibimiz.</em></>}>Mühendislikten satışa, projelerimizi birlikte hayata geçiren ekip.</PageHero><section><div className="team-grid full-team">{team.map(person => <article key={person.id || person.name}>{person.image ? <img src={person.image} alt={person.name}/> : <div className="team-placeholder" aria-label={`${person.name} fotoğrafı yakında eklenecek`}>{person.name?.split(" ").map(part => part[0]).join("").slice(0, 2)}</div>}<h3>{person.name}</h3><p>{person.role}</p></article>)}</div></section></main>; }
